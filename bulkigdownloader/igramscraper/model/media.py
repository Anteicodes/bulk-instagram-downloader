...
class Media(InitializerModel):
    ...
    def _init_properties_custom(self, value, prop, arr):
        ...
        if prop in standart_properties:
            ...
            urls = []
            for mek in arr[prop]['edges']:
                if mek['node']['__typename'] == 'GraphVideo':
                    urls.append(mek['node']['video_url'])
                    self.tipeh = Media.TYPE_VIDEO
                elif mek['node']['__typename'] == 'GraphImage':
                    urls.append(mek['node']['display_url'])
                    self.tipeh = Media.TYPE_IMAGE

            if self.tipeh == Media.TYPE_IMAGE:
                self.image_standard_resolution_url = urls
            elif self.tipeh == Media.TYPE_VIDEO:
                self.video_standard_resolution_url = urls
            # #TODO implement
